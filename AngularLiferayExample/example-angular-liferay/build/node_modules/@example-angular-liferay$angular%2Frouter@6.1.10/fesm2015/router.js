Liferay.Loader.define('@example-angular-liferay$angular/router@6.1.10/fesm2015/router', ['module', 'exports', 'require'], function (module, exports, require) {
    ...
    EmptyOutletComponent.decorators = [{ type: Component, args: [{ template: `<router-outlet></router-outlet>` }] }];
    ...
    function equalParamsAndUrlSegments(a, b) {
        ...
        return equalUrlParams && !parentsMismatch && (!a.parent || equalParamsAndUrlSegments(a.parent, /** @type {?} */b.parent));
    }
    ...
    class PreActivation {
        ...
        resolveData(paramsInheritanceStrategy) {
            ...
            return from(this.canActivateChecks).pipe(concatMap(check => this.runResolve(check.route, paramsInheritanceStrategy)), reduce((_, __) => _));
        }
        /**
         * @return {?}
         */
        ...
        runCanDeactivateChecks() {
            return from(this.canDeactivateChecks).pipe(mergeMap(check => this.runCanDeactivate(check.component, check.route)), every(result => result === true));
        }
        /**
         * @return {?}
         */
        runCanActivateChecks() {
            return from(this.canActivateChecks).pipe(concatMap(check => andObservables(from([this.fireChildActivationStart(check.route.parent), this.fireActivationStart(check.route), this.runCanActivateChild(check.path), this.runCanActivate(check.route)]))), every(result => result === true));
        ...
        runCanActivateChild(path) {
            ...
            /** @type {?} */
            const canActivateChildGuards = path.slice(0, path.length - 1).reverse().map(p => this.extractCanActivateChild(p)).filter(_ => _ !== null);
        ...
        runCanDeactivate(component, curr) {
            ...
            const canDeactivate$ = from(canDeactivate).pipe(mergeMap(c => {
                ...
                if (guard.canDeactivate) {
                    observable = wrapIntoObservable(guard.canDeactivate(component, curr, this.curr, this.future));
                } else {
    ...
    function match$1(segmentGroup, route, segments) {
        ...
        /** @type {?} */
        const parameters = res.consumed.length > 0 ? Object.assign({}, posParams, res.consumed[res.consumed.length - 1].parameters) : posParams;
    ...
    class Router {
        ...
        setUpLocationChangeListener() {
            ...
            if (!this.locationSubscription) {
                this.locationSubscription = /** @type {?} */this.location.subscribe(change => {
                    ...
                    /** @type {?} */
                    const state = change.state && change.state.navigationId ? { navigationId: change.state.navigationId } : null;
                    setTimeout(() => {
         ...
         */
        processNavigations() {
            this.navigations.pipe(concatMap(nav => {
                if (nav) {
        ...
        executeScheduledNavigation({ id, rawUrl, extras, resolve, reject, source, state }) {
            ...
            if ((this.onSameUrlNavigation === 'reload' ? true : urlTransition) && this.urlHandlingStrategy.shouldProcessUrl(rawUrl)) {
                ...
                /** @type {?} */this.events.next(new NavigationStart(id, this.serializeUrl(url), source, state));
                Promise.resolve().then(_ => this.runNavigate(url, rawUrl, !!extras.skipLocationChange, !!extras.replaceUrl, id, null)).then(resolve, reject);
                // we cannot process the current URL, but we could process the previous one =>
                // we need to do some cleanup
            } else if (urlTransition && this.rawUrlTree && this.urlHandlingStrategy.shouldProcessUrl(this.rawUrlTree)) {
                /** @type {?} */this.events.next(new NavigationStart(id, this.serializeUrl(url), source, state));
                Promise.resolve().then(_ => this.runNavigate(url, rawUrl, false, false, id, createEmptyState(url, this.rootComponentType).snapshot)).then(resolve, reject);
            } else {
        ...
        runNavigate(url, rawUrl, skipLocationChange, replaceUrl, id, precreatedState) {
            if (id !== this.navigationId) {
                /** @type {?} */this.events.next(new NavigationCancel(id, this.serializeUrl(url), `Navigation ID ${id} is not equal to the current navigation id ${this.navigationId}`));
            ...
            return new Promise((resolvePromise, rejectPromise) => {
                ...
                if (!precreatedState) {
                    ...
                    urlAndSnapshot$ = redirectsApplied$.pipe(mergeMap(appliedUrl => {
                        return recognize(this.rootComponentType, this.config, appliedUrl, this.serializeUrl(appliedUrl), this.paramsInheritanceStrategy, this.relativeLinkResolution).pipe(map(snapshot => {
                            /** @type {?} */this.events.next(new RoutesRecognized(id, this.serializeUrl(url), this.serializeUrl(appliedUrl), snapshot));
                ...
                const preactivationDone$ = preactivationResolveData$.pipe(mergeMap(p => {
                    if (typeof p === 'boolean' || this.navigationId !== id) return of(false);
                    return this.hooks.afterPreactivation(p.snapshot, {
                        navigationId: id,
                        appliedUrlTree: url,
                        rawUrlTree: rawUrl, skipLocationChange, replaceUrl
                    }).pipe(map(() => p));
        ...
        activateRoutes(state, storedState, storedUrl, id, url, rawUrl, skipLocationChange, replaceUrl, resolvePromise, rejectPromise) {
            ...
            state.forEach(p => {
                ...
                new ActivateRoutes(this.routeReuseStrategy, state, storedState, evt => this.triggerEvent(evt)).activate(this.rootContexts);
                navigationIsSuccessful = true;
            }).then(() => {
                if (navigationIsSuccessful) {
                    this.navigated = true;
                    this.lastSuccessfulId = id;
                    /** @type {?} */this.events.next(new NavigationEnd(id, this.serializeUrl(url), this.serializeUrl(this.currentUrlTree)));
                    resolvePromise(true);
                } else {
                    this.resetUrlToCurrentUrlTree();
                    /** @type {?} */this.events.next(new NavigationCancel(id, this.serializeUrl(url), ''));
                ...
                if (isNavigationCancelingError(e)) {
                    ...
                    /** @type {?} */this.events.next(new NavigationCancel(id, this.serializeUrl(url), e.message));
                    resolvePromise(false);
                } else {
                    this.resetStateAndUrl(storedState, storedUrl, rawUrl);
                    /** @type {?} */this.events.next(new NavigationError(id, this.serializeUrl(url), e));
                    try {
    ...
    /** @nocollapse */
    RouterLink.ctorParameters = () => [{ type: Router }, { type: ActivatedRoute }, { type: String, decorators: [{ type: Attribute, args: ['tabindex'] }] }, { type: Renderer2 }, { type: ElementRef }];
    ...
    /** @nocollapse */
    RouterLinkActive.ctorParameters = () => [{ type: Router }, { type: ElementRef }, { type: Renderer2 }, { type: ChangeDetectorRef }];
    ...
    RouterOutlet.decorators = [{ type: Directive, args: [{ selector: 'router-outlet', exportAs: 'outlet' }] }];
    /** @nocollapse */
    RouterOutlet.ctorParameters = () => [{ type: ChildrenOutletContexts }, { type: ViewContainerRef }, { type: ComponentFactoryResolver }, { type: String, decorators: [{ type: Attribute, args: ['name'] }] }, { type: ChangeDetectorRef }];
    ...
    /** @type {?} */
    const ROUTER_PROVIDERS = [Location, { provide: UrlSerializer, useClass: DefaultUrlSerializer }, {
        provide: Router,
    ...
    RouterModule.decorators = [{ type: NgModule, args: [{
            declarations: ROUTER_DIRECTIVES,
            exports: ROUTER_DIRECTIVES,
            entryComponents: [EmptyOutletComponent]
        }] }];
    /** @nocollapse */
    RouterModule.ctorParameters = () => [{ type: undefined, decorators: [{ type: Optional }, { type: Inject, args: [ROUTER_FORROOT_GUARD] }] }, { type: Router, decorators: [{ type: Optional }] }];
    /**
     * @param {?} router
     * @param {?} viewportScroller
     * @param {?} config
